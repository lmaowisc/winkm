...
-   **Summary functions**
    -   **Joint survival**: $H_a(t, s)=\pr(T_a>t, D_a> s)$ - likely unavailable
    -   **OS**: $S_a(t) =\pr(D_a > t)$ - available through Kaplan-Meier (KM) curve
    -   **EFS**: $S_a^*(t) =\pr(T_a^* > t)$ - available through KM curve
...
-   **Win-loss probabilities** [@oakes:2016]\begin{align}
     ...
     win on nonfatal}}\notag\\
     &= \int_0^\tau \color{blue}{S_a(t-)\d F_{1-a}(t)} 
     \quad\quad\quad\quad \big(F_a(t) = 1 - S_a(t)\big)\\
     &\hspace{1em}+\color{blue}{S_1(\tau)S_0(\tau)} \int_0^\tau \color{red}{\pr(T_a >t\mid D_a>\tau) \pr(t\leq T_{1-a} <t +\d t\mid D_{1-a}>\tau)}
    \end{align}
...
-   **Second-term unknown** \begin{align}
    &\int_0^\tau \color{red}{\pr(T_a >t\mid D_a>\tau) \pr(t\leq T_{1-a} <t +\d t\mid D_{1-a}>\tau)}\\
    =& -\int_0^\tau \color{red}{H_a(t\mid \tau) H_{1-a}(\d t\mid \tau)}\\
    =&\,\color{red}{\pr(T_{1-a}<T_a\wedge \tau\mid D_1 > \tau, D_0 >\tau)}
    \end{align}
    -   $H_a(t\mid \tau)=\pr(T_a>t\mid D_a>\tau)$: event-free probabilities in $\tau$-survivors
...
-   **Start with** $t$-survivors \begin{align}
    \d\Lambda_a(t\mid\tau) &:= \pr(t\le T_a < t + \d t\mid T_a\geq t, D_a>\color{red}{\tau})\\
    &\leq \pr(t\le T_a < t + \d t\mid T_a\geq t, D_a\geq \color{red}{t})\\
    &\approx \frac{N_a^{\rm E}}{N_a^*}\d\hat\Lambda_a^*(u)
    \end{align}
    -   Notation
...
-   **Local dependence** [@oakes:1982; @oakes:1986] \begin{align*}\label{eq:cr:def}
      \kappa_a(t, s) &=\frac{\pr(t\leq T_a< t +\d t\mid T_a\geq t,  D_a = s )}
    {\pr(t\leq T_a< t +\d t\mid T_a\geq t, D_a \geq s)}  \notag\\
...
-   **Estimating association parameter** [@mao:2022] $$
    ...
    $$
    -   $\hat r_a^{\rm E}=N_a^{\rm D}/L_a^{\rm OS}$: nonfatal event rate
    -   $\hat r_a^{\rm D}= N_a^{\rm E}/L_a^*$: death rate
...
-   **Formula** $$
    H_a(t\mid\tau) \approx \prod_{0\leq u \leq t}\left(1 - \frac{N_a^{\rm E}}{N_a^*}\d\hat\Lambda_a^*(u)\underbrace{\prod_{u\leq s \leq \tau}
       \left[1 - \{\hat\kappa_a(u, s)-1\}\frac{\d\hat F_a(s)}{\hat S_a(s)}\right]}_{\text{Bias correction for $\tau$-survivorship}}\right)
    $$
...
-   A **step-by-step** approach
    ...
    -   `compute_increments()`: calculate $\d\hat S_a(t)$ and $\d\hat S^*_a(t)$
...
-   **`WinKM` Website**
    ...
    >  Calculating win-loss statistics using summary data: [https://lmaowisc.github.io/winKM/](https://lmaowisc.github.io/winKM/)
