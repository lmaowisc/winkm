...
-   **Win-loss probabilities** [@oakes:2016]\begin{align}
     ...
     win on nonfatal}}\notag\\
     &= \int_0^\tau \color{blue}{S_a(t-){\rm d} F_{1-a}(t)} 
     \quad\quad\quad\quad \big(F_a(t) = 1 - S_a(t)\big)\\
     &\hspace{1em}+\color{blue}{S_1(\tau)S_0(\tau)} \int_0^\tau \color{red}{\mathrm{pr}(T_a >t\mid D_a>\tau) \mathrm{pr}(t\leq T_{1-a} <t +{\rm d} t\mid D_{1-a}>\tau)}
    \end{align}
...
-   **Second-term unknown** \begin{align}
    &\int_0^\tau \color{red}{\mathrm{pr}(T_a >t\mid D_a>\tau) \mathrm{pr}(t\leq T_{1-a} <t +{\rm d} t\mid D_{1-a}>\tau)}\\
    =& -\int_0^\tau \color{red}{H_a(t\mid \tau) H_{1-a}({\rm d} t\mid \tau)}\\
...
-   **Start with** $t$-survivors \begin{align}
    {\rm d}\Lambda_a(t\mid\tau) &:= \mathrm{pr}(t\le T_a < t + {\rm d} t\mid T_a\geq t, D_a>\color{red}{\tau})\\
    &\leq \mathrm{pr}(t\le T_a < t + {\rm d} t\mid T_a\geq t, D_a\geq \color{red}{t})\\
    &\approx \frac{N_a^{\rm E}}{N_a^*}{\rm d}\hat\Lambda_a^*(u)
    \end{align}
    -   Notation
...
-   **Local dependence** [@oakes:1982; @oakes:1986] \begin{align*}\label{eq:cr:def}
      \kappa_a(t, s) &=\frac{\mathrm{pr}(t\leq T_a< t +{\rm d} t\mid T_a\geq t,  D_a = s )}
    {\mathrm{pr}(t\leq T_a< t +{\rm d} t\mid T_a\geq t, D_a \geq s)}  \notag\\
...
-   **Formula** $$
    H_a(t\mid\tau) \approx \prod_{0\leq u \leq t}\left(1 - \frac{N_a^{\rm E}}{N_a^*}{\rm d}\hat\Lambda_a^*(u)\underbrace{\prod_{u\leq s \leq \tau}
       \left[1 - \{\hat\kappa_a(u, s)-1\}\frac{{\rm d}\hat F_a(s)}{\hat S_a(s)}\right]}_{\text{Bias correction for $\tau$-survivorship}}\right)
    $$
...
-   A **step-by-step** approach
    ...
    -   `compute_increments()`: calculate ${\rm d}\hat S_a(t)$ and ${\rm d}\hat S^*_a(t)$
